...
包内任何文件（含测试）不得 import `common/`、`internal/`、`constant/`、
`types/`、`dto/` 或其他业务包。
...
- `cachex/`: 底层缓存原语。当前唯一调用方是 `service/channel_affinity.go`，
...
- 依赖核查必须看符号级依赖而非仅 import 语句：候选文件即使直接 import 只有
  ...
  阶段 2 对 `common/body_storage.go` 的核查结论（暂留 `common/`，随阶段 3/4 迁
