...
- `common/AGENTS.md` — JSON 包装、静态资源、URL/缓存安全边界。
- `router/AGENTS.md` — 路由分层、web 静态资源、SSE 保护。
- `controller/AGENTS.md` — HTTP 边界、输入校验、**审计埋点检查清单**、使用日志字段可见性。
- `middleware/AGENTS.md` — 认证/限速/分发/请求体恢复/SSE 保护。
- `service/AGENTS.md` — 业务逻辑、**`service.RecordAudit` 签名与行为约定**、计费/配额。
- `model/AGENTS.md` — GORM、**SQLite/MySQL/PostgreSQL 三库兼容**、缓存、幂等迁移。
- `setting/AGENTS.md` — 配置注册、默认值校验、**审计配置**。
- `relay/AGENTS.md` — AI 中继、协议转换、流式输出/usage/计费保护、供应商适配。
- `i18n/AGENTS.md` — 后端响应消息多语言(go-i18n,`i18n.Msg*` 常量,locales/en.yaml + zh.yaml)。
- `docs/AGENTS.md` — 文档规则。
