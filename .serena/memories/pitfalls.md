...
**规则**:`internal/store/AGENTS.md` — 必须同时兼容 SQLite / MySQL >= 5.7.8 / PostgreSQL >= 9.6。用 `db.UsingPostgreSQL`/`UsingSQLite`/`UsingMySQL`(`internal/infra/db/database.go`)分支处理差异,优先 GORM,原始 SQL 必须参数化。
