...
func Recharge(referenceId string, customerId string) (err error) {
	...
	RecordLog(topUp.UserId, LogTypeTopup, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%d", log.FormatQuota(int(quota)), topUp.Amount))
...
func CompleteEpayTopUp(tradeNo string, paymentMethod string, paidMoney string) error {
	...
	RecordLog(userId, LogTypeTopup, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", log.LogQuota(quotaToAdd), payMoney))
	return nil
}
...
func ManualCompleteTopUp(tradeNo string) error {
	...
	// 事务外记录日志，避免阻塞
	RecordLog(userId, LogTypeTopup, fmt.Sprintf("管理员补单成功，充值金额: %v，支付金额：%f", log.FormatQuota(quotaToAdd), payMoney))
	return nil
}
