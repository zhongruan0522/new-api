...
func (r *DynamicRatioRule) Validate() error {
	...
	if !ratio.ContainsGroupRatio(r.Group) {
...
func GetDynamicRatioRules() ([]*DynamicRatioRule, error) {
	...
	err := dbstore.DB.Order("priority ASC, id ASC").Find(&rules).Error
	return rules, err
}
...
func GetDynamicRatioRuleById(id int64) (*DynamicRatioRule, error) {
	...
	err := dbstore.DB.Where("id = ?", id).First(&rule).Error
	return &rule, err
}
