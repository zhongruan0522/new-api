...
func GetRandomSatisfiedChannelByAPIType(p RandomSatisfiedChannelByAPITypeParams) (*Channel, error) {
	...
	if len(ids) == 0 {
		normalized := ratio.FormatMatchingModelName(p.ModelName)
...
func getChannelByAPITypeDBWithModelName(q channelByAPITypeDBQuery) (*Channel, error) {
	...
	err = dbstore.DB.Model(&Ability{}).
		...
		Where("abilities."+dbstore.CommonGroupCol+" = ? AND abilities.model = ? AND abilities.enabled = ? AND abilities.priority = ? AND channels.status = ? AND channels.type IN (?)",
	...
	var ch Channel
	err = dbstore.DB.First(&ch, "id = ?", channelID).Error
	if err != nil {
		return nil, err
	}
	return &ch, nil
...
func listPrioritiesByAPIType(group string, modelName string, allowedTypes []int) ([]int, error) {
	...
	err := dbstore.DB.Model(&Ability{}).
		...
		Joins("JOIN channels ON channels.id = abilities.channel_id").
		Where("abilities."+dbstore.CommonGroupCol+" = ? AND abilities.model = ? AND abilities.enabled = ? AND channels.status = ? AND channels.type IN (?)",
