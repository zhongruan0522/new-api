...
func setupSetupTestDB(t *testing.T) {
	...
	if err := db.AutoMigrate(&userstore.User{}, &Setup{}); err != nil {
...
func newSetupRootUser(name string) *userstore.User {
...
func TestInitializeSetupConcurrentInstancesCreateSingleRoot(t *testing.T) {
	...
	roots := make([]*userstore.User, instances)
	...
	// 只有一个 root 用户
	var rootCount int64
	if err := dbstore.DB.Model(&userstore.User{}).Where("role = ?", common.RoleRootUser).Count(&rootCount).Error; err != nil {
	...
	if err := dbstore.DB.Model(&Setup{}).Count(&setupCount).Error; err != nil {
...
func TestInitializeSetupRollsBackWhenRootCreationFails(t *testing.T) {
	...
	if err := dbstore.DB.Create(existing).Error; err != nil {
