...
func TestUpdateTokenUsedQuotaLeavesRemainQuotaUnchanged(t *testing.T) {
	...
	var got Token
	if err := dbstore.DB.First(&got, token.Id).Error; err != nil {
...
func TestUpdateTokenUsedQuotaOnlyUpdatesTargetToken(t *testing.T) {
	...
	if err := dbstore.DB.Create(&target).Error; err != nil {
		t.Fatalf("create target token: %v", err)
	}
	if err := dbstore.DB.Create(&other).Error; err != nil {
	...
	var gotTarget Token
	if err := dbstore.DB.First(&gotTarget, target.Id).Error; err != nil {
		t.Fatalf("load target token: %v", err)
	}
	var gotOther Token
	if err := dbstore.DB.First(&gotOther, other.Id).Error; err != nil {
