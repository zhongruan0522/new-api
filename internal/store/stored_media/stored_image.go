...
func (img *StoredImage) Insert(ctx context.Context) error {
	...
	return dbstore.DB.WithContext(ctx).Create(img).Error
}
...
func GetStoredImageByID(ctx context.Context, id string) (*StoredImage, error) {
	...
	var img StoredImage
	if err := dbstore.DB.WithContext(ctx).Where("id = ?", id).First(&img).Error; err != nil {
		return nil, err
	}
	return &img, nil
}
...
func GetStoredImageByUserAndSha(ctx context.Context, userId int, sha256 string) (*StoredImage, error) {
	...
	var img StoredImage
	if err := dbstore.DB.WithContext(ctx).Where("user_id = ? AND sha256 = ?", userId, sha256).Order("created_at asc").First(&img).Error; err != nil {
		return nil, err
	}
	return &img, nil
}
...
func GetStoredImageMetaByID(ctx context.Context, id string) (*StoredImage, error) {
	...
	var img StoredImage
	if err := dbstore.DB.WithContext(ctx).Model(&StoredImage{}).
...
func DeleteStoredImagesInRange(ctx context.Context, startTimestamp, endTimestamp int64, limit int) (int64, error) {
	...
	for {
		...
		tx := dbstore.DB.WithContext(ctx).Where("created_at <= ?", endTimestamp)
		if startTimestamp > 0 {
...
func EnsureStoredImagesPoolLimit(ctx context.Context, maxBytes int64, batchSize int) (int64, error) {
	...
	for {
		...
		if err := dbstore.DB.WithContext(ctx).Model(&StoredImage{}).Select("coalesce(sum(size_bytes),0)").Scan(&totalBytes).Error; err != nil {
			return deleted, err
		}
		...
		result := dbstore.DB.WithContext(ctx).Where("id IN ?", ids).Delete(&StoredImage{})
		if result.Error != nil {
