...
func (v *StoredVideo) Insert(ctx context.Context) error {
	...
	return dbstore.DB.WithContext(ctx).Create(v).Error
}
...
func GetStoredVideoByID(ctx context.Context, id string) (*StoredVideo, error) {
	...
	var v StoredVideo
	if err := dbstore.DB.WithContext(ctx).Where("id = ?", id).First(&v).Error; err != nil {
		return nil, err
	}
	return &v, nil
}
...
func GetStoredVideoByUserAndSha(ctx context.Context, userId int, sha256 string) (*StoredVideo, error) {
	...
	var v StoredVideo
	if err := dbstore.DB.WithContext(ctx).Where("user_id = ? AND sha256 = ?", userId, sha256).Order("created_at asc").First(&v).Error; err != nil {
		return nil, err
	}
	return &v, nil
}
...
func GetStoredVideoMetaByID(ctx context.Context, id string) (*StoredVideo, error) {
	...
	var v StoredVideo
	if err := dbstore.DB.WithContext(ctx).Model(&StoredVideo{}).
...
func EnsureStoredVideosPoolLimit(ctx context.Context, maxBytes int64, batchSize int) (int64, error) {
	...
	for {
		...
		if err := dbstore.DB.WithContext(ctx).Model(&StoredVideo{}).Select("coalesce(sum(size_bytes),0)").Scan(&totalBytes).Error; err != nil {
			return deleted, err
		}
		...
		result := dbstore.DB.WithContext(ctx).Where("id IN ?", ids).Delete(&StoredVideo{})
		if result.Error != nil {
