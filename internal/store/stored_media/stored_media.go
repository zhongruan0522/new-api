...
func queryStoredMedia(ctx context.Context, userId int, startTimestamp, endTimestamp int64, startIdx, pageSize int) ([]StoredMediaListItem, int64, error) {
	...
	if err := dbstore.DB.WithContext(ctx).Raw(countSQL, unionArgs...).Scan(&total).Error; err != nil {
		return nil, 0, err
	}
	...
	if err := dbstore.DB.WithContext(ctx).Raw(listSQL, listArgs...).Scan(&items).Error; err != nil {
		return nil, 0, err
	}
...
func GetMediaConvertStatsByUserId(userId int, startTime, endTime int64) (MediaConvertStats, error) {
	...
	err := dbstore.DB.Model(&StoredImage{}).
		Where("user_id = ? AND created_at >= ? AND created_at <= ?", userId, startTime, endTime).
		Count(&stats.ImageCount).Error
	if err != nil {
		return stats, err
	}
	err = dbstore.DB.Model(&StoredVideo{}).
		Where("user_id = ? AND created_at >= ? AND created_at <= ?", userId, startTime, endTime).
