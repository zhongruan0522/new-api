...
func (v *Vendor) Insert() error {
	...
	v.CreatedTime = now
	v.UpdatedTime = now
	return dbstore.DB.Create(v).Error
}
...
func IsVendorNameDuplicated(id int, name string) (bool, error) {
	...
	var cnt int64
	err := dbstore.DB.Model(&Vendor{}).Where("name = ? AND id <> ?", name, id).Count(&cnt).Error
	return cnt > 0, err
}
...
func (v *Vendor) Update() error {
	...
	return dbstore.DB.Save(v).Error
}

// Delete 软删除供应商
func (v *Vendor) Delete() error {
	return dbstore.DB.Delete(v).Error
}

// GetVendorByID 根据 ID 获取供应商
func GetVendorByID(id int) (*Vendor, error) {
	var v Vendor
	err := dbstore.DB.First(&v, id).Error
	if err != nil {
		return nil, err
	}
	return &v, nil
