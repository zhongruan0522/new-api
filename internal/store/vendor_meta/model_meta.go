...
func (mi *Model) Insert() error {
	...
	if err := dbstore.DB.Create(mi).Error; err != nil {
		return err
	}

	// 使用保存的原始值进行更新，确保零值能正确保存
	return dbstore.DB.Model(&Model{}).Where("id = ?", mi.Id).Updates(map[string]interface{}{
...
func IsModelNameDuplicated(id int, name string) (bool, error) {
	...
	var cnt int64
	err := dbstore.DB.Model(&Model{}).Where("model_name = ? AND id <> ?", name, id).Count(&cnt).Error
	return cnt > 0, err
}
...
func (mi *Model) Update() error {
	...
	return dbstore.DB.Model(&Model{}).Where("id = ?", mi.Id).
...
func (mi *Model) Delete() error {
	return dbstore.DB.Delete(mi).Error
}
...
func GetAllModels(offset int, limit int) ([]*Model, error) {
	var models []*Model
	err := dbstore.DB.Order("id DESC").Offset(offset).Limit(limit).Find(&models).Error
	return models, err
}
...
func SearchModels(keyword string, vendor string, offset int, limit int) ([]*Model, int64, error) {
	var models []*Model
	db := dbstore.DB.Model(&Model{})
	if keyword != "" {
		like := "%" + keyword + "%"
		db = db.Where("model_name LIKE ? OR tags LIKE ?", like, like)
	}
	if vendor != "" {
