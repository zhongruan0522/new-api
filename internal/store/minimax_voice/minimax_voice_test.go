...
func TestDeleteExpiredMiniMaxVoicePreviews_DeletesOnlyExpiredPreviews(t *testing.T) {
	...
	// 验证过期试听已被删除。
	var cnt int64
	if err := dbstore.DB.Model(&MiniMaxVoice{}).Where("voice_id = ?", "expired-preview-1").Count(&cnt).Error; err != nil {
...
func TestDeleteExpiredMiniMaxVoicePreviews_BoundaryKeepsExactlyCutoffAge(t *testing.T) {
	...
	var cnt int64
	if err := dbstore.DB.Model(&MiniMaxVoice{}).Where("voice_id = ?", "boundary-preview").Count(&cnt).Error; err != nil {
