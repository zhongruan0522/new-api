...
func IsMiniMaxVoiceIdExists(voiceId string) (bool, error) {
	...
	var cnt int64
	err := dbstore.DB.Model(&MiniMaxVoice{}).Where("voice_id = ?", voiceId).Count(&cnt).Error
	return cnt > 0, err
}
...
func UpdateMiniMaxVoiceType(id int64, newType string) error {
	return dbstore.DB.Model(&MiniMaxVoice{}).Where("id = ?", id).
...
func ConfirmMiniMaxVoice(id int64, operatorId int, quotaCost int) (bool, error) {
	...
	tx := dbstore.DB.Model(&MiniMaxVoice{}).
...
func DeleteMiniMaxVoiceById(id int64) error {
	return dbstore.DB.Delete(&MiniMaxVoice{}, "id = ?", id).Error
}
...
func DeleteExpiredMiniMaxVoicePreviews(cutoff int64) (int64, error) {
	...
	res := dbstore.DB.Where("type = ? AND created_at < ?", MiniMaxVoiceTypePreview, cutoff).
		Delete(&MiniMaxVoice{})
...
func CountMiniMaxVoices() (int64, error) {
	var cnt int64
	if err := dbstore.DB.Model(&MiniMaxVoice{}).Count(&cnt).Error; err != nil {
		return 0, err
	}
	return cnt, nil
}
