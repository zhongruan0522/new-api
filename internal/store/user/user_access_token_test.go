...
func TestValidateAccessTokenRejectsEmptyBearerWhenEmptyTokenExists(t *testing.T) {
	...
	for _, authorization := range []string{"", "   ", "Bearer", "Bearer ", "bearer "} {
		t.Run(fmt.Sprintf("authorization_%q", authorization), func(t *testing.T) {
			...
			if err != dbstore.ErrTokenInvalid {
				t.Fatalf("ValidateAccessToken(%q) err = %v, want %v", authorization, err, dbstore.ErrTokenInvalid)
			}
			if got != nil {
...
func TestValidateAccessTokenRejectsUnknownTokenWithoutRecordNotFoundLog(t *testing.T) {
	...
	if err != dbstore.ErrTokenInvalid {
		t.Fatalf("ValidateAccessToken() err = %v, want %v", err, dbstore.ErrTokenInvalid)
	}
	if got != nil {
...
func TestCleanupEmptyAccessTokensConvertsEmptyStringToNull(t *testing.T) {
	...
	var root User
	if err := dbstore.DB.First(&root, 1).Error; err != nil {
	...
	var commonUser User
	if err := dbstore.DB.First(&commonUser, 2).Error; err != nil {
