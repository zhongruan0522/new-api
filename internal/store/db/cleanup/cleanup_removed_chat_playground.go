...
func CleanupRemovedChatPlaygroundData() error {
	...
	if optionstore.IsDataMigrationDone(markerRemovedChatPlayground) {
		return nil
	}
	...
	// ---------------------------------------------------------------------
	if res := dbstore.DB.Delete(&optionstore.Option{Key: "Chats"}); res.Error != nil {
		return res.Error
	} else if res.RowsAffected > 0 {
		common.SysLog("removed obsolete option: Chats")
	}
	for _, k := range []string{"ChatLink", "ChatLink2"} {
		if res := dbstore.DB.Delete(&optionstore.Option{Key: k}); res.Error != nil {
			return res.Error
		} else if res.RowsAffected > 0 {
	...
	result := dbstore.DB.
		...
		FindInBatches(&users, 200, func(tx *gorm.DB, _ int) error {
			for i := range users {
				...
				if err := tx.Model(&userstore.User{}).Where("id = ?", u.Id).Update("setting", sanitizedSetting).Error; err != nil {
					return err
				}
				updatedUsers++
			}
	...
	optionstore.MarkDataMigrationDone(markerRemovedChatPlayground)
	return nil
}
