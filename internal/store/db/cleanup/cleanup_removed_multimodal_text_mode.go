...
func CleanupRemovedMultimodalTextMode() error {
	...
	if optionstore.IsDataMigrationDone(markerRemovedMultimodalTextMode) {
		return nil
	}
	...
	for _, key := range removedThirdPartyMultimodalOptionKeys {
		res := dbstore.DB.Delete(&optionstore.Option{Key: key})
		if res.Error != nil {
	...
	result := dbstore.DB.Model(&channelstore.Channel{}).
		Select("id", "settings").
		...
		FindInBatches(&channels, 200, func(tx *gorm.DB, _ int) error {
			for i := range channels {
				...
				if err := tx.Model(&channelstore.Channel{}).Where("id = ?", ch.Id).Update("settings", normalized).Error; err != nil {
	...
	optionstore.MarkDataMigrationDone(markerRemovedMultimodalTextMode)
	return nil
}
