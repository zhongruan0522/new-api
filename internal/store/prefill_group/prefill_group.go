...
func (g *PrefillGroup) Insert() error {
	...
	g.CreatedTime = now
	g.UpdatedTime = now
	return dbstore.DB.Create(g).Error
}
...
func IsPrefillGroupNameDuplicated(id int, name string) (bool, error) {
	...
	var cnt int64
	err := dbstore.DB.Model(&PrefillGroup{}).Where("name = ? AND id <> ?", name, id).Count(&cnt).Error
	return cnt > 0, err
}
...
func (g *PrefillGroup) Update() error {
	...
	return dbstore.DB.Save(g).Error
}

// DeleteByID 根据 ID 删除组
func DeletePrefillGroupByID(id int) error {
	return dbstore.DB.Delete(&PrefillGroup{}, id).Error
}
...
func GetAllPrefillGroups(groupType string) ([]*PrefillGroup, error) {
	...
	query := dbstore.DB.Model(&PrefillGroup{})
	if groupType != "" {
