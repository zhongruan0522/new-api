...
func TestExtractChannelAffinityValueFromRequestHeader(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			src := operation.ChannelAffinityKeySource{
				Type: "request_header",
				Key:  tc.key,
			}
...
func TestGetPreferredChannelByAffinityFromRequestHeader(t *testing.T) {
	...
	origRules := make([]operation.ChannelAffinityRule, len(setting.Rules))
	copy(origRules, setting.Rules)

	setting.Enabled = true
	setting.Rules = []operation.ChannelAffinityRule{
		{
			Name:       "trace-by-header",
			ModelRegex: []string{"^gpt-.*$"},
			PathRegex:  []string{"/v1/chat/completions"},
			KeySources: []operation.ChannelAffinityKeySource{
