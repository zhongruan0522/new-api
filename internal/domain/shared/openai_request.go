...
func createFileSource(data string) *FileSource {
	if strings.HasPrefix(data, "http://") || strings.HasPrefix(data, "https://") {
		return NewURLFileSource(data)
	}
	return NewBase64FileSource(data, "")
}

func (r *GeneralOpenAIRequest) GetTokenCountMeta() *TokenCountMeta {
	var tokenCountMeta TokenCountMeta
	var texts = make([]string, 0)
	var fileMeta = make([]*FileMeta, 0)
...
func (m *Message) ParseToolCalls() []ToolCallRequest {
	...
	if err := jsonx.Unmarshal(m.ToolCalls, &toolCalls); err == nil {
		return toolCalls
	}
	return toolCalls
}
...
func (m *Message) SetToolCalls(toolCalls any) {
	toolCallsJson, _ := jsonx.Marshal(toolCalls)
...
/*func (m *Message) StringContent() string {
	...
	if err := jsonx.Unmarshal(m.Content, &stringContent); err == nil {
...
func (m *Message) SetStringContent(content string) {
	jsonContent, _ := jsonx.Marshal(content)
...
func (m *Message) ParseContent() []MediaContent {
	...
	// 尝试解析为数组
	var arrayContent []map[string]interface{}
	if err := jsonx.Unmarshal(m.Content, &arrayContent); err == nil {
...
func (r *OpenAIResponsesRequest) GetTokenCountMeta() *TokenCountMeta {
	var fileMeta = make([]*FileMeta, 0)
	var texts = make([]string, 0)
