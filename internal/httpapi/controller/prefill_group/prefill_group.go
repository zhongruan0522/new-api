...
func CreatePrefillGroup(c *gin.Context) {
	...
	if g.Name == "" || g.Type == "" {
		httpapi.ApiErrorI18n(c, i18n.MsgPrefillGroupNameTypeRequired)
		return
	}
	// 创建前检查名称
	if dup, err := prefillgroupstore.IsPrefillGroupNameDuplicated(0, g.Name); err != nil {
		...
		httpapi.ApiErrorI18n(c, i18n.MsgPrefillGroupNameExists)
		return
	}
...
func UpdatePrefillGroup(c *gin.Context) {
	...
	if g.Id == 0 {
		httpapi.ApiErrorI18n(c, i18n.MsgPrefillGroupMissingID)
		return
	}
	// 查询更新前的原始数据用于审计差异对比
