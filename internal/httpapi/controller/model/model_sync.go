	...
	for attempt := 0; attempt < attempts; attempt++ {
		...
		func() {
			...
			case http.StatusOK:
				...
				if err := jsonx.Unmarshal(buf, out); err != nil {
					// Try decode as pure array
					var arr []T
					if err2 := jsonx.Unmarshal(buf, &arr); err2 != nil {
						lastErr = err
						return
					}
					out.Success = true
...
func ensureVendorID(vendorName string, vendorByName map[string]upstreamVendor, vendorIDCache map[string]int, createdVendors *int) int {
	...
	var existing vendormetastore.Vendor
	if err := dbstore.DB.Where("name = ?", vendorName).First(&existing).Error; err == nil {
		vendorIDCache[vendorName] = existing.Id
		return existing.Id
	}
	uv := vendorByName[vendorName]
	v := &vendormetastore.Vendor{
		Name:        vendorName,
...
func SyncUpstreamModels(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.T(c, i18n.MsgModelSyncFetchUpstreamFailed) + ": " + fetchErr.Error(), "locale": req.Locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
	...
	for _, name := range missing {
		...
		var existing vendormetastore.Model
		if err := dbstore.DB.Where("model_name = ?", name).First(&existing).Error; err == nil {
	...
	if len(req.Overwrite) > 0 {
		...
		for _, ow := range req.Overwrite {
			...
			var local vendormetastore.Model
			if err := dbstore.DB.Where("model_name = ?", ow.ModelName).First(&local).Error; err != nil {
				continue
			}
			...
			// 应用字段覆盖（事务）
			_ = dbstore.DB.Transaction(func(tx *gorm.DB) error {
				needUpdate := false
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.T(c, i18n.MsgModelSyncFetchUpstreamFailed) + ": " + fetchErr.Error(), "locale": locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
	...
	// 2) 本地已有模型
	var locals []vendormetastore.Model
	if len(upstreamNames) > 0 {
		_ = dbstore.DB.Where("model_name IN ? AND sync_official <> 0", upstreamNames).Find(&locals).Error
	}
	...
	if len(vendorIDs) > 0 {
		var dbVendors []vendormetastore.Vendor
		_ = dbstore.DB.Where("id IN ?", vendorIDs).Find(&dbVendors).Error
	...
	// 3) 缺失且上游存在的模型
	missingList, _ := missingmodelstore.GetMissingModels()
	var missing []string
