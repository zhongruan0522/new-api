...
func GetFileTypeFromUrl(c *gin.Context, url string, reason ...string) (string, error) {
	...
	if response.StatusCode != 200 {
		log.LogError(c, fmt.Sprintf("failed to download file from %s, status code: %d", url, response.StatusCode))
	...
	for _, limit := range limits {
		log.LogDebug(c, fmt.Sprintf("Trying to read %d bytes to determine file type", limit))
