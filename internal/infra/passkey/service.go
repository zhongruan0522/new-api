...
func resolveOrigins(r *http.Request, settings *system.PasskeySettings) ([]string, error) {
...
autoDetect:
	...
	if host == "" && system.ServerAddress != "" {
		if parsed, err := url.Parse(system.ServerAddress); err == nil && parsed.Host != "" {
			host = parsed.Host
	...
	if host == "" {
		return nil, fmt.Errorf("无法确定 Passkey 的 Origin，请在系统设置或 Passkey 设置中指定。当前 Host: '%s', ServerAddress: '%s'", r.Host, system.ServerAddress)
	}
	if scheme == "" {
		scheme = "https"
	}
...
func resolveRPID(r *http.Request, settings *system.PasskeySettings, origins []string) (string, error) {
