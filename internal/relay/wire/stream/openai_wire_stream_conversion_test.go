...
func TestChatToResponsesStreamConverter_BuffersToolCallUntilNameKnown(t *testing.T) {
	...
	raw, err := jsonx.Marshal(firstChunk)
	if err != nil {
	...
	raw, err = jsonx.Marshal(secondChunk)
	if err != nil {
...
func TestChatToResponsesStreamConverter_CustomToolCallInput(t *testing.T) {
	converter := NewChatToResponsesStreamConverter()
	custom, err := jsonx.Marshal(map[string]any{"name": "code_exec", "input": "print("})
	if err != nil {
	...
	custom, err = jsonx.Marshal(map[string]any{"input": "\n    "})
	if err != nil {
	...
	custom, err = jsonx.Marshal(map[string]any{"input": "1)"})
	if err != nil {
...
func TestChatToResponsesStreamConverter_CustomToolProxyInputDone(t *testing.T) {
	toolContext := convert.NewOpenAIWireToolContext()
	toolContext.AddCustomToolProxy("shell_exec", "shell_exec")
	converter := NewChatToResponsesStreamConverter(toolContext)

	arguments, err := convert.BuildChatArgumentsForResponsesCustomToolInput("printf 'hi'\n")
	if err != nil {
