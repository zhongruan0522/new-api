...
func (s *chatToResponsesToolCallState) responsesArguments() (string, error) {
	...
	raw := s.args.String()
	if s.customProxy {
		input, complete := convert.ExtractResponsesCustomToolInputFromChatArguments(raw)
		if !complete {
...
func (s *chatToResponsesToolCallState) applyToolSpec(spec convert.OpenAIWireToolSpec) {
	if s == nil {
		return
	}
	s.toolSpec = spec
...
func (s *chatToResponsesToolCallState) responsesOutputItem(status string) (shared.ResponsesOutput, error) {
	...
	if s.isToolSearch() {
		...
		if status == "completed" {
			arguments, err := convert.BuildResponsesToolSearchArgumentsFromChatArguments(s.args.String())
			if err != nil {
...
func (c *chatToResponsesStreamConverter) buildUsage() *shared.Usage {
	...
	return convert.MapChatUsageToResponsesUsage(*c.usage)
}
