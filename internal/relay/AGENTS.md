...
- 计费边界：usage → quota 的计算与落账只经 `domain/billing.CalculateUsage(ctx, relayInfo, rawUsage, extraContent...)` + `ApplyQuota(ctx, relayInfo, settlement)`（通用文本路径）或既有 `PostClaudeConsumeQuota` / `PostAudioConsumeQuota` / Pre&PostWss 等领域侧入口完成；阶段 4 起这些入口的 quota 一律由归一化 `BillingUsage` 和 `billing.CalculateNormalizedQuotaForRelay` 计算，不再从聚合字段反推普通输入，也不得绕过价格表/旧配置固定优先级；relay 各层不得 import store 的 token/user/channel 包读写配额。
