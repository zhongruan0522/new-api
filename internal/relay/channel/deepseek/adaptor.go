...
func applyDeepSeekV4OpenAIThinkingSuffix(info *relaycommon.RelayInfo, request *shared.GeneralOpenAIRequest) error {
...
func applyDeepSeekV4ClaudeThinkingSuffix(info *relaycommon.RelayInfo, request *shared.ClaudeRequest) error {
	modelName := request.Model
	if info != nil && info.ChannelMeta != nil && info.UpstreamModelName != "" {
		modelName = info.UpstreamModelName
	}
	...
	request.Model = baseModel
	request.Thinking = &shared.Thinking{Type: thinkingType}
	if effort == "" {
