...
const DIRECTION_OPTIONS = {
  mysql: [{ value: 'mysql_to_postgres', label: 'MySQL -> PostgreSQL' }],
  postgres: [{ value: 'postgres_to_mysql', label: 'PostgreSQL -> MySQL' }],
  sqlite: [
    { value: 'sqlite_to_postgres', label: 'SQLite -> PostgreSQL' },
    { value: 'sqlite_to_mysql', label: 'SQLite -> MySQL' },
  ],
};

function inferTargetType(direction) {
  if (direction?.endsWith('_to_postgres')) return 'postgres';
  if (direction?.endsWith('_to_mysql')) return 'mysql';
  return '';
}

function validateTargetDsn(direction, dsn) {
  const target = inferTargetType(direction);
...
const DatabasePreMigration = () => {
  ...
  const fetchInfo = async () => {
    setInfoLoading(true);
    try {
      const res = await API.get('/api/db/pre_migrate/info', {
      ...
      setInfo(data);
      const mainType = data?.main_db_type || '';
      const opts = DIRECTION_OPTIONS[mainType] || [];
      if (opts.length > 0) setDirection(opts[0].value);
    } catch (e) {
      showError(e);
    } finally {
  ...
  const fetchJob = async (id) => {
    ...
    setJobLoading(true);
    try {
      const res = await API.get(`/api/db/pre_migrate/${id}`, {
        disableDuplicate: true,
      });
      const { success, message, data } = res.data;
      if (!success) {
      ...
      if (data?.status && data.status !== 'running') {
        stopPolling();
        if (data.status === 'success') showSuccess(t('预迁移完成'));
        if (data.status === 'failed') showError(data?.error || t('预迁移失败'));
      }
    } catch (e) {
      showError(e);
    } finally {
  ...
  const handleOpenConfirm = () => {
    const msg = validateTargetDsn(direction, targetDsn);
    if (msg) {
      showError(t(msg));
      return;
    }
  ...
  const handleStart = async () => {
    ...
    try {
      const res = await API.post('/api/db/pre_migrate', {
  ...
  const targetType = inferTargetType(direction);

  return (
    <Card>
      <Form.Section text={t('数据库预迁移')}>
        <Banner
          type='warning'
          description={t(
            '该功能用于把当前数据库的业务数据复制到目标数据库。未勾选“覆盖目标库”时，要求目标库必须为空；勾选后会清空目标库的相关表再迁移。请务必提前备份目标数据库。',
          )}
            ...
            <Col xs={24} sm={24} md={12} lg={12} xl={12}>
              ...
              <div style={{ marginTop: 8 }}>
                <Select
                  ...
                  style={{ width: '100%' }}
                  optionList={DIRECTION_OPTIONS[mainType] || []}
                  placeholder={t('请选择迁移方向')}
                />
              </div>
              <div style={{ marginTop: 8 }}>
                <Text type='tertiary'>
                  {t('目标数据库类型')}：{DB_TYPE_LABEL[targetType] || '-'}
                </Text>
              </div>
            </Col>
