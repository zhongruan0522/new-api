...
| Package Name | Channel Type | API Address (Enter Identifier) |
| --- | --- | --- |
| Zhipu GLM Official Plan | Zhipu GLM 4V | `glm-coding-plan` |
| Zhipu GLM International Plan | Zhipu GLM 4V | `glm-coding-plan-international` |
...
| Xiaomi CN Plan | Xiaomi | `xiaomi-coding-plan` |
| Xiaomi SGP Plan | Xiaomi | `xiaomi-coding-plan-sgp` |
| Xiaomi AMS Plan | Xiaomi | `xiaomi-coding-plan-ams` |

The actual upstream addresses corresponding to each plan identifier are defined in `ChannelSpecialBases` in `constant/channel.go`. After creating a plan channel, you can query plan quota usage from the channel list (currently supported for GLM, Kimi, and MiniMax).

Zhipu GLM plans (`glm-coding-plan` / `glm-coding-plan-international`) support three protocol endpoints, routed automatically by request protocol:
