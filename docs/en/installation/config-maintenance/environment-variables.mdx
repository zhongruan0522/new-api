...
| `SQL_MAX_IDLE_CONNS`    | Maximum Number of Idle Connections in Connection Pool | `20`                     | `SQL_MAX_IDLE_CONNS=50`                                                                                                          |
...
| `RELAY_MAX_IDLE_CONNS`           | Maximum idle connections for the relay HTTP client           | `200`         | `RELAY_MAX_IDLE_CONNS=100`           |
| `RELAY_MAX_IDLE_CONNS_PER_HOST`  | Maximum idle connections per host for the relay HTTP client  | `32`          | `RELAY_MAX_IDLE_CONNS_PER_HOST=16`   |
| `RELAY_IDLE_CONN_TIMEOUT`        | Relay HTTP client idle connection retention time (seconds)   | `90`          | `RELAY_IDLE_CONN_TIMEOUT=30`         |
| `SYNC_HTTP_MAX_IDLE_CONNS`       | Maximum idle connections for the model metadata sync client  | `10`          | `SYNC_HTTP_MAX_IDLE_CONNS=20`        |
| `SYNC_HTTP_IDLE_CONN_TIMEOUT`    | Idle connection timeout for the model metadata sync client (seconds) | `30`   | `SYNC_HTTP_IDLE_CONN_TIMEOUT=60`     |
| `STREAMING_TIMEOUT`              | Streaming Single Reply Timeout (seconds)                     | `300`         | `STREAMING_TIMEOUT=120`              |
| `STREAM_SCANNER_MAX_BUFFER_MB`   | Maximum buffer size for a single streaming SSE frame (MB)    | `8`           | `STREAM_SCANNER_MAX_BUFFER_MB=16`    |
