...
| `SQL_PREPARE_STMT`      | 启用 GORM 预编译语句缓存（适合高频 SQL 路径） | `false`                  | `SQL_PREPARE_STMT=true`                                                                                                         |
...
| `RELAY_MAX_IDLE_CONNS`         | 中继HTTP客户端最大空闲连接数 | `200`  | `RELAY_MAX_IDLE_CONNS=100`        |
| `RELAY_MAX_IDLE_CONNS_PER_HOST` | 中继HTTP客户端每Host最大空闲连接数 | `32` | `RELAY_MAX_IDLE_CONNS_PER_HOST=16` |
| `RELAY_IDLE_CONN_TIMEOUT`      | 中继HTTP客户端空闲连接保留时间(秒) | `90` | `RELAY_IDLE_CONN_TIMEOUT=30`      |
| `SYNC_HTTP_MAX_IDLE_CONNS`     | 模型元数据同步客户端最大空闲连接数 | `10` | `SYNC_HTTP_MAX_IDLE_CONNS=20`     |
| `SYNC_HTTP_IDLE_CONN_TIMEOUT`  | 模型元数据同步客户端空闲连接保留时间(秒) | `30` | `SYNC_HTTP_IDLE_CONN_TIMEOUT=60`  |
| `STREAMING_TIMEOUT`            | 流式一次回复的超时时间(秒)  | `300`  | `STREAMING_TIMEOUT=120`           |
| `STREAM_SCANNER_MAX_BUFFER_MB` | 单个流式SSE帧最大缓冲(MB)   | `8`    | `STREAM_SCANNER_MAX_BUFFER_MB=16` |
