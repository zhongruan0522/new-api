...
| --- | --- | --- |
| 智谱 GLM 官方套餐 | 智谱 GLM 4V | `glm-coding-plan` |
| 智谱 GLM 国际套餐 | 智谱 GLM 4V | `glm-coding-plan-international` |
...
| 小米国内套餐 | 小米 | `xiaomi-coding-plan` |
| 小米新加坡套餐 | 小米 | `xiaomi-coding-plan-sgp` |
| 小米荷兰套餐 | 小米 | `xiaomi-coding-plan-ams` |

套餐标识对应的实际上游地址定义在 `constant/channel.go` 的 `ChannelSpecialBases` 中。套餐渠道创建后，可在渠道列表中查询套餐额度使用情况（目前支持 GLM、Kimi、MiniMax）。

智谱 GLM 套餐（`glm-coding-plan` / `glm-coding-plan-international`）支持三种协议端点，系统按请求协议自动路由：
