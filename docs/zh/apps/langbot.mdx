...
<Callout type="info">
  ...
  应用能力，并支持通过 OpenAI 兼容接口接入 NookMux。
</Callout>
...
LangBot 支持接入本地部署的 NookMux 或任何 OpenAI 兼容的 NookMux 服务。

### 使用方式

1.  从 NookMux 中获取 API key
    ![获取 API key](/assets/langbot/get_api_key.png)

        若是本地部署的 NookMux 请自行配置 API 地址（可参考[容器网络连接](https://docs.langbot.app/zh/workshop/network-details.html)），若使用第三方 NookMux 服务，可在页面上复制地址。注意，地址后需要添加`/v1`。

2.  在 LangBot 中添加模型，选择使用 NookMux 供应商，填写对应的 API key 和 API 地址
    ![添加 NookMux 模型](/assets/langbot/add_newapi_model.png)
...
LangBot 支持使用 NookMux 的嵌入模型，并将其作为知识库的向量模型。

1. 在 LangBot 中添加嵌入模型，选择使用 NookMux 供应商
