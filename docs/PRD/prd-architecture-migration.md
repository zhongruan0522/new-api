...
> **执行记录（2026-08-21，阶段 4 落地）**：`internal/common/` 32 个源文件 + 5 个测试文件 + `limiter/` 子包迁出（git mv 保留历史），`internal/constant/` 整包解散，190 个文件完成符号改写。按 PRD 映射落地 `infra/db`（database.go）、`infra/redis`（redis.go）、`infra/cache`（disk_cache*+body_storage+limiter）、`infra/email`（email*）、`infra/security`（ssrf/ip/url_validator/crypto/totp/verification/validate/trusted_proxies）、`infra/runtime`（system_monitor*/pprof/pyro/gopool*/go_channel）、`app/env.go`（init.go）、`app/webdist`（embed_file_system.go）、`httpapi` 根包（gin.go+request_body_limit.go）、`config/operation`（condition.go，唯一调用方 tool_billing）、`relay/helper`（response.go）；`sys_log.go`、`env.go`（GetEnvOrDefault*）、hash/copy/str/utils/rate_limit 及 PRD 列明的暂留文件留守 `internal/common/`。执行中偏离原映射的内容（均为依赖成环或前提失效，与 5.1–5.4 同类）：
> 1. **`sys_log.go` 留守 `internal/common/` 而非并入 `infra/log`**（PRD 未预见的环）：既有 `infra/log/logger.go` 依赖 `common.LogDir`/`DebugEnabled`/`QuotaPerUnit`/`RelayGo`；若 `SysLog` 家族迁入 `infra/log`，则留守的 `timezone.go`/`topup_ratio.go`（用 `SysError`）与 `infra/log → common` 互成环。消除全部环需把 `DebugEnabled`/`QuotaPerUnit` 等业务全局迁出 common 或做多处注入反转，代价大于收益。
> 2. **`ErrRequestBodyTooLarge`/`IsRequestBodyTooLargeError` 自 gin.go 移入 `infra/cache`**（PRD 未细分）：二者被 body_storage 的磁盘路径直接返回/判断，若留在 httpapi 会形成 `cache → httpapi → cache` 环；错误语义归存储层，httpapi 的 `GetRequestBody` 引用 `cache.ErrRequestBodyTooLarge` 再包装具体上限。
> 3. **`constant/context_key.go` 落 `internal/common` 而非 5.1 计划的 `internal/httpapi`**（环）：`ContextKey` 被 domain（billing/channel）、store/user、infra（tokenizer/media/log）、relay 全层引用，落 httpapi 根包会使底层反向依赖 HTTP 边界聚合层；且 `infra/log → httpapi → shared → infra/log`（shared/gemini.go 既有依赖）三方成环。`common` 是跨层内核（现仅 import `domain/channel/constant`+`pkg/jsonx`），`ContextKey` 即"跨领域全局常量"，与 common 阶段 4 后的定位一致。
> 4. **`constant/env.go`/`cache_key.go`/`setup.go` 按计划落 `domain/shared`**，前提是 `common → constant` 依赖随 gin.go/request_body_limit.go/url_validator.go/init.go/response.go 全部迁出而消解（已验证 common 现仅 import `domain/channel/constant` 与 `pkg/jsonx`，`shared → common` 保持单向无环）。
> 5. **启动 flag（`--port`/`--version`/`--help`/`--log-dir`）随 `init.go` 落 `app/env.go`，`--log-dir` 值经 `infra/log.Dir` 变量注入**（PRD 未预见）：`logger.go` 需要日志目录但不能反向 import app；由 `InitEnv` 在 flag.Parse 后赋值，单一写者。`env.go` 的 `GetEnvOrDefault*` 留守 common（infra/redis、infra/runtime、store/db 等底层包在用，落 app 即成环）。
> 6. **`hash.go`/`copy.go`/`str.go`/`utils.go`/`rate_limit.go` 评估后留守 common**（PRD 允许"随调用方归位"）：调用方分散于 domain/infra/relay/store 各层无单一归属；`str.go` 依赖业务开关 `DebugEnabled` 不满足 `pkg/` 门槛，`utils.go` 含 `OpenBrowser`（exec）等应用层行为；与"common 只剩业务全局变量和零碎工具"的阶段目标一致。顺手将 `utils.go`/`topup_ratio.go` 残留的 `encoding/json` 直调改为 `jsonx`（jsonx 为纯透传，零语义差异）。
> 7. **符号级适配（逻辑零变更）**：`internal/constant` 解散后其引用按内容分流——`ContextKey*` → `common.*`，运行时限值/缓存键/`Setup` → `shared.*`；`common.RelayGo` → `runtime.RelayGo` 等 190 文件按符号映射表改写；`domain/channel/constant` 与 `relay/constant`（包名亦为 `constant`）的既有引用不受影响（先以别名 `channelconstant`/plain import 区分，机械改写时对该两类文件做符号白名单还原）。`requestInterval` 非导出中间变量随 `InitEnv` 迁 app 改为局部变量。
>
> 依赖方向（迁移后关键边，全部无环）：`common → {domain/channel/constant, pkg/jsonx}`；`infra/* → {common, shared}`（security→shared 读 `TrustedRedirectDomains`；log→runtime 用 `RelayGo`；runtime→cache 读磁盘缓存路径）；`httpapi根 → {infra/cache, domain/shared, common}`；`shared → {common, infra/log}`（dto/types 既有）；domain/store/relay 各层按需 import `httpapi` 根包取 gin 边界工具（根包为叶子工具包，不含 router/middleware/controller）。AGENTS.md 同步：根（结构概览/constant 解散说明）、common（全重写：内核定位+分层规则+sys_log 留守论证）、infra（新增 6 子包与依赖方向及 tokenizer→httpapi 根包例外）、domain（shared 段落按解散结果重写）、app/pkg/router 各处路径；用户文档（中英文技术架构、本地开发结构树、环境变量解析路径）与 `.serena/memories` 同步；`docs/差异性/` 为历史分析记录，沿用 5.2–5.4 口径不回改。验证：`go build ./...` 全绿；`go test -count=1 ./...` 64 个测试包全过、0 失败（较 5.4 记录的 60 包新增 email/security/runtime/httpapi 根包等含测试新包；5.4 遗留的 user 包基线失败项已在后续修复中关闭）；测试函数 719=719（与本次迁移前基线一致，`^func Test` 口径）；`gofmt -l` 干净；`go vet ./...` 干净；`go list -deps ./pkg/...` 无业务依赖；`go list -deps ./internal/store/...` 无 controller/middleware/router 反向依赖；全仓无 `internal/constant` 与 `internal/common/limiter` 残留引用。
...
> **执行记录（2026-08-21，阶段 5.5 落地）**：relay 顶层 29 个文件全部收口（git mv 保留历史）：`core/`（relay_adaptor + openrouter 测试、websocket、stored_asset_signature + 测试、stored_image_url）、`wire/`（`auto_convert.go` + `auto_convert_helpers.go`，由 `openai_wire_auto_convert*.go` 更名）、`wire/stream/`（根包 stream writer/capture writer/转换测试 + `common/` 的 3 个 `openai_wire_stream_*`）、`handler/`（10 个模态 handler + 测试 + 3 个 `third_party_media_to_text*` 墓碑文件）；`common/` 的 11 个 `openai_wire_convert_*` 与 `openai_wire_tool_context.go` 迁 `wire/convert/`，`common/` 留守 billing/image_handling/media_text_handling 及 relay_info/override/reasoning_effort/openrouter_routing/outbound_body/relay_utils/request_conversion（PRD「留」清单未穷举，均非 openai_wire_*，不动）。执行中偏离/补充原映射的内容（均为依赖约束或 PRD 未列出，与 5.1–5.4 同类）：
> 1. **根包改薄门面 `relay.go`**（PRD 目标树未列根包文件，但「不动外部 import 路径」要求根包 API 不变）：re-export 原根包对外符号共 14 个（`GetAdaptor`/`WssHelper`/10 个模态 handler 入口含 `ResponsesHelper`/`RelayStoredImage`/`RelayStoredVideo`/`OpenAIWireHelper`），其中外部 4 个调用方（`httpapi/router/relay_router.go`、`httpapi/controller/{relay,model,channel}`）实际使用 13 个（`ResponsesHelper` 由 wire 内部按 RelayMode 分发，无外部直连，保留 re-export 以维持原导出面完整），外部调用方零改动。
> 2. **`openai_wire_tool_context.go` 落 `wire/convert/` 而非 wire 根**（PRD 未点名）：convert 函数签名携带 `*OpenAIWireToolContext`，若落 wire 根则 wire/convert ↔ wire 互相 import 成环；留守 `relay_info.go` 的 `OpenAIResponsesToolContext` 字段引用其类型，形成 `common → wire/convert` 单向边，wire/convert 因此保持不依赖任何 relay 内部包的叶子（依赖 `domain/shared`、`internal/common` 的 `Interface2String` 工具与 `pkg/jsonx`，后者自迁移前原样存在）。
> 3. **`stored_image_url.go` 随 `stored_asset_signature*` 落 `core/`**（PRD 未列出）：同属存储资产签名 URL 域；唯一调用方是 handler（compatible/stored_image/stored_video），`verifyStoredAssetSignature`/`buildStoredImageURL`/`buildStoredVideoURL` 相应导出（`VerifyStoredAssetSignature`/`BuildStoredImageURL`/`BuildStoredVideoURL`），构成 `handler → core` 边。
> 4. **删除 `openai_wire_stream_chat_to_responses_compat.go`**：其存在意义是根包非导出接口与 relaycommon 导出接口间的适配垫片；两套接口合并进 wire/stream 后方法集完全一致，保留导出的 `OpenAIWireStreamConverter`，writer 直接使用具体转换器构造函数。符号级适配（逻辑零变更，唯一调用方 wire 包）：`openAIWireStreamWriter`→`StreamWriter`、`openAIWireStreamOptions`→`StreamOptions`、`newOpenAIWireStreamWriter`→`NewStreamWriter`、`openAIWireCaptureWriter`→`CaptureWriter`、`newOpenAIWireCaptureWriter`→`NewCaptureWriter`。
> 5. **`openai_wire_concurrency_benchmark_test.go` 转 `stream_test` 外部测试包**：其基准对象是 `openai.Adaptor` 全链路且 import `relay/channel/openai`，而后者反向依赖 wire/stream 的流式转换器，作为包内测试会构成 import cycle。
> 6. **`message_reasoning_test.go`（仅测 shared DTO marshal 保留显式空 reasoning_content）随 compatible_handler 落 `handler/`**；`compatible_handler_test.go` 实测 `billing.CalculateStreamSpeed`（domain/billing 函数），按历史分组随迁不改归属。
> 7. **依赖方向（迁移后，全部无环）**：`relay(门面) → {core, wire, handler}`；`wire → {handler, common, helper, wire/convert, wire/stream}`；`handler → {core, common, helper, constant}`；`core → {channel/*, common, helper, billing, shared, security}`；`common → wire/convert`；`wire/stream → wire/convert`；`channel/claude`、`channel/openai` 的 `relaycommon.OpenAIWire*` 引用（共 6 文件 15 处限定符改写 + 8 行 import）改指 `wire/convert`/`wire/stream`；`common/relay_info.go` 的字段类型改指 `wire/convert`。
>
> AGENTS.md 同步：`internal/relay/AGENTS.md`（新增包结构一节与依赖方向，转换归口规则改为「OpenAI wire 一律放 wire/convert、wire/stream」）、根 `AGENTS.md` 结构概览 relay 条目；`docs/差异性/`、`docs/PRD` 关联 PRD 为历史分析记录不回改；用户文档引用的 `relay/common/override.go` 留守未变。验证：`go build ./...` 全绿；`go test -count=1 ./...` 67 个测试包全过、0 失败（阶段 4 记录 64 包 + 新增 core/handler/wire/convert/wire/stream 4 个含测试新包 − relay 根包不再有测试文件 = 67）；relay 范围测试函数 305=305、全仓 `^func (Test|Benchmark|Fuzz)` 761=761（迁移前后一致，无丢失）；`gofmt -l` 干净；`go vet ./internal/relay/... ./internal/httpapi/...` 干净。
...
> **执行记录（2026-08-21，阶段 6 落地）**：PRD 所指「`relay/common/billing.go` 的 usage 计算逻辑」经调研已不在该文件——5.3 阶段该文件仅剩 `BillingSettler` 消费侧接口（实现在 `domain/billing.BillingSession`），真正的 usage 计算主体是 `relay/handler/compatible_handler.go` 的私有 `postConsumeQuota`（356 行，直接 import store/user、store/channel、store/log 写配额与消费日志）。据此按 PRD 意图执行：
> 1. **计费边界**：`postConsumeQuota` 整体下沉 `domain/billing/usage.go`，拆为 `CalculateUsage(ctx, relayInfo, rawUsage, extraContent...) (*UsageSettlement, *shared.NookMuxError)`（纯计算：usage 兜底、渠道亲和度观察、上下文阶梯计价、decimal 倍率乘加、web/file/image/audio 工具费、空 usage 可重试错误判定）+ `ApplyQuota(ctx, relayInfo, settlement)`（落账：用户/渠道已用计数、SettleBilling 结算、OtherInfo 与消费日志）。`UsageSettlement` 为不透明结果载体（字段不导出）。9 个 handler 调用点（compatible/gemini×2/embedding/rerank/image/responses×2/audio）改为成对调用；`compatible_handler.go` 据此摆脱 store/channel、store/log 及计费性的 store/user 依赖（保留 `userstore.IncrementMediaConvertedCount`——媒体转换计数非配额表）。Claude/Audio/WSS 路径的 `PostXxxConsumeQuota` 已在 domain 侧且被 IR PRD 列为待合并项，本阶段不动其形状。新增 `usage_test.go` 7 个用例锁定计算语义（缓存扣除 ×2 语义、按次计费、预估兜底、空 usage 重试/继续、地板价）。
> 2. **协议转换边界**：调研确认算法只有一份（wire/convert），散的是触发点；且 `core → channel/*` 既有依赖使 channel 包无法 import wire 根包（会成环），故单点入口落 `wire/convert/converter.go` 的 `Converter` 会话类型（channel 已能到达）：承载 Upstream/Downstream 方向与 ToolContext 生命周期，方法覆盖请求双向与非流式响应双向转换。收编 5 个文件 6 处直调（auto_convert 两流各 1、openai/adaptor ConvertClaudeRequest、claude/adaptor ConvertOpenAIResponsesRequest、claude/relay_claude 非流式响应、openai/relay_responses 非流式响应）；wire 层 `openAIWireConversionOptions` 并入 Converter 后删除（StreamWriter/CaptureWriter 编排改从会话取方向与选项）；流式因依赖方向（stream→convert）无法收进 convert，维持 `NewChatToResponsesStreamConverter` / `NewResponsesToChatStreamConverter` 两构造器为流式单点；`ApplyResponsesUsageToChatUsage` 为无状态叶子原语保留直调。外部引用面收敛为 `{NewConverter, ApplyResponsesUsageToChatUsage, OpenAIWireToolContext 类型} ∪ stream{两构造器, OpenAIWireStreamConverter 类型}`。新增 `converter_test.go` 4 个用例验证 ToolContext 捕获/回填反解。
> 3. **保真度**：搬移代码逐行等价（归一化 diff 复核）：空 usage 三分支语义经 `totalTokensZero`/`hasToolFees` 标记在 ApplyQuota 内按原语句顺序重放（有工具费时计数写入→LogError）；logContent 保持「extraContent 累积 → gizmo 分支追加 → 尾部 strings.Join」的原顺序。已核实 `ApplyContextPricingResult` 不改写 GroupRatioInfo，ApplyQuota 重读分组倍率与计算阶段快照一致。
> 4. **依赖方向**（`go list` 复核，无环）：usage.go 新增的生产依赖边仅 `billing → config/operation`；`domain/channel`、`httpapi 根`为 billing 包既有边（log_info.go / usage_helper.go 在用；domain/channel 不反向依赖 billing 主包，httpapi 根为叶子工具包）。AGENTS.md 同步：relay（common 条目改 BillingSettler 接口定位、wire 条目补 Converter 单点、规则区补转换单点与计费边界两条）、domain（billing 子包表补 usage.go、依赖方向按 go list 实测重写）。
>
> 验证：`go build ./...` 全绿；`go test -count=1 ./...` 67 个测试包全过、0 失败；测试函数 772 = 761（阶段 5.5 基线）+ 11（新增 usage_test 7 + converter_test 4），无丢失；`gofmt -l` 干净；`go vet ./...` 干净；channel 层对 wire 的引用面经 grep 复核收敛至预期清单。
>
> **复查记录（2026-08-21，阶段 6 审查修正）**：对 08d4497d 做双轨复查（主线逐段归一化 diff + 前台子代理并行保真度审查）。等价性确认：计费 356 行逐语句对照无语义差异（空 usage 三分支副作用顺序、extraContent 追加顺序、OtherInfo 全字段、`ApplyContextPricingResult` 不写 GroupRatioInfo 均经源码验证）；6 处转换收编点方向参数与 ToolContext 链路（adaptor 捕获 → RelayInfo 中转 → relay_claude 回填）与基线等价；converter_test 4 用例经 overlay 变异测试证实具备回归防护；外部引用面逐符号与预期清单吻合。两处修正：① ApplyQuota 的 originalGroupRatio 基数由「落账时从 PriceData 重读」改为随 settlement 快照计价前值，恢复迁移前单函数的读取时点（当前全链路无写者、行为零变化，仅消除未来演化偏差窗口）；② 上文第 4 条「新增边」表述失实——`domain/channel`、`httpapi 根`在基线 billing 包已存在，真正新增的生产边仅 `config/operation`，已改写。测试计数 772=761+11 经同口径（`git ls-tree` + `^func (Test|Benchmark|Fuzz)`）复测确认无误。
