...
新旧两套前端调用的是完全相同的后端 API（`/api/*`、`/v1/*`）。换 UI 不等于换接口。

上游参考项目中 `web/default/` 和 `web/classic/` 共用同一个后端，API 路径、请求/响应格式完全一致。
...
- relay 层：不改
- middleware 层：不改
- 业务路由（api-router、dashboard-router、relay-router）：不改
...
- `main.go` — embed 指令调整
- `router/web-router.go` — 根据主题设置返回对应的静态资源
- `router/main.go` — 函数签名适配
- `common/embed-file-system.go` — 新增 ThemeAwareFS 能力
- `common/constants.go` — 新增主题原子变量
- `setting/system_setting/` — 新增主题配置持久化模块
- `controller/misc.go` — 在已有的 `/api/status` 响应中追加 `theme` 字段（1 行）
...
前端通过已有的 `/api/status` 接口获取当前主题值（`theme` 字段）。不新建接口。

### 5.2 管理员切换

管理员在系统设置中选择前端主题（default / classic），通过已有的 `/api/option` 接口保存。保存后立即生效，无需重启服务。

### 5.3 默认值策略

- 新部署 / 升级：默认 `classic`（用户无感）
- 后续 default 稳定后可改为默认 `default`
...
以下为之前删除的 `ui-optimization-plan.csv` 内容，记录 classic 主题已完成的优化：

| TaskID | Page/Component | Description | Priority | Status |
|--------|---------------|-------------|----------|--------|
| T01 | SectionPageLayout | 创建统一的页面布局组件 | High | Done |
| T02 | PageTransition | 添加页面切换过渡动画 | Medium | Done |
| T03 | Skeleton Loading | 添加骨架屏shimmer动画 | Medium | Done |
| T04 | EmptyState | 创建统一的空状态组件 | Medium | Done |
| T05 | Dashboard | StatsCards交错入场动画 | High | Done |
| T06-14 | Pages | 10个页面统一使用SectionPageLayout | High | Done |
| T15 | Micro-interactions | 卡片悬停/按钮缩放/表格行交错动画 | Medium | Done |
| T16 | Global Styles | 渐变遮罩/数字等宽/减少动画偏好 | Medium | Done |
| T17 | i18n | 补充11个翻译key | Medium | Done |
| T18 | Pricing | 加入场动画class | High | Done |
| T19 | Home | 模块化拆分为HeroSection+ProviderIcons | High | Done |
| T20 | Mobile | 移动端卡片视图(CardTable) | Medium | Done |
| T21 | Build | 编译通过 | High | Done |

这些优化已在 classic 主题中完成，不需要在 default 主题中重复实现（default 有自己的设计体系）。
