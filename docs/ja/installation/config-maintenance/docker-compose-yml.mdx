...
services:
  new-api:
    ...
    environment:
      ...
      - REDIS_CONN_STRING=redis://:123456@redis:6379 # ⚠️ 本番環境では必ずこのパスワードを変更してください！
      - TZ=Asia/Shanghai
  ...
  redis:
    ...
    restart: always
    command: ['redis-server', '--requirepass', '123456'] # ⚠️ 本番環境では必ずこのパスワードを変更してください！
...
| `REDIS_CONN_STRING` | Redis 接続文字列                                     | `redis://:123456@redis:6379`          |
